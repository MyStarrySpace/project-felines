...
function firstAuthorLastName(authors: string): string {
  ...
  // Filter out initials: tokens that are all uppercase and ≤3 chars (e.g. "A", "DS", "MF", "SY-Y")
  // Also filter tokens that are single chars with periods like "A."
  ...
  // Return first non-initial token only (the last name)
  // For multi-word last names like "La Rosa", keep the first token
...
async function fetchPubMed(pmid: string): Promise<ApiResult> {
  ...
  try {
    ...
    // PubMed format: "LastName Initials" -> extract last name
...
async function fetchCrossRef(doi: string): Promise<ApiResult> {
  ...
  try {
    const res = await fetch(url, {
      headers: {
        "User-Agent":
          "PLIG-Framework/1.0 (bibliography-verification; mailto:noreply@example.com)",
      },
    });
    if (!res.ok) return { error: `HTTP ${res.status}` };
    const data = await res.json();
